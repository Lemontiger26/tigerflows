<script lang="ts">
	...
	let { step, ontoggle, oncomment, readonly = false }: Props = $props();

	let showComment = $state(false);

	function toggleComment(e: Event) {
		e.stopPropagation();
	...
	function circleClass(): string {
		if (step.checked) return 'bg-success/20 border-2 border-success text-success';
		if (step.isCritical) return 'bg-error/15 border-2 border-error text-error';
	...
	function cardClass(): string {
		...
		if (step.checked) return `${base} bg-success/10 border-l-4 border-success shadow-inner scale-[0.98] ${interactive}`;
		if (step.isCritical) return `${base} bg-error/5 border-l-4 border-error shadow-sm ${interactive}`;
		...
		<div class="card-body p-3">
			<div class="flex items-start gap-2">
				<div class="flex-1 min-w-0">
					<h4 class="font-medium text-base">{step.title}</h4>
					{#if step.description}
						<div class="mt-1">
							<MarkdownView content={step.description} class="text-sm text-base-content/70" />
						</div>
					{/if}
				</div>
				<div class="flex items-center gap-1 shrink-0">
					{#if step.checked && step.checkedAt}
						<span class="text-sm text-success">{formatTime(step.checkedAt)}</span>
					{/if}
					{#if !readonly}
						<button
							type="button"
...
{#if showComment}
	<div class="pl-11 pb-2">
		<textarea
			...
			onblur={(e) => oncomment((e.target as HTMLTextAreaElement).value)}
			rows="2">{step.comment ?? ''}</textarea
		>
	</div>
{/if}
